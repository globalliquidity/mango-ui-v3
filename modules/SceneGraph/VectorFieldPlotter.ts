...
export class VectorFieldPlotter extends SolidParticleSystemElement {
  ...
  async onCreate() {
    ...
    this.meshBase.scaling = new bjs.Vector3(0, 0, 0)
    this.meshBase.setEnabled(true)
  }

  public addCell(cell: VectorFieldCell, rebuildMesh = false) {
    this.sps.addShape(this.meshBase, 1)

    if (rebuildMesh) this.sps.rebuildMesh()
  }
  ...
  protected onUpdateParticle = (particle: bjs.SolidParticle) => {
    //Logger.log("VectorField : onUpdateParticle");

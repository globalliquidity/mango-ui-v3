...
export class VectorFieldPresenter extends SceneElementPresenter<PresenterUpdateMessage> {
  ...
  getCurrentRowLayer(
    ...
    //Logger.log("DepthFinderPresenter : Get Current Row Layer");
    if (this.rows.length > 0) {
      //Logger.log(this.rows);
      const currentRow: VectorFieldRow | undefined = this.getCurrentRow()

      if (currentRow) {
        const layer: VectorFieldLayer | undefined =
          currentRow.getLayer(layerType)

        if (layer) {
          return layer
        } else {
          // Logger.log("DepthFinderPresenter : Get Current Row Layer : Can't get layer");
        }
      } else {
        Logger.log(
          "DepthFinderPresenter : Get Current Row Layer : Can't get current row"
        )
      }
    } else {
      Logger.log('DepthFinderPresenter : Get Current Row Layer : No Rows')
      //return null;
    }

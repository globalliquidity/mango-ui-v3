...
export class VectorFieldRow {
  ...
  layers: Map<VectorFieldLayerType, VectorFieldLayer> = new Map<
  ...
  getLayer(layerType: VectorFieldLayerType): VectorFieldLayer | undefined {
    if (this.layers.has(layerType)) return this.layers.get(layerType)
  }
  ...
  getCellByPrice(
    ...
    //Logger.log("VectorFieldRow : Get Cell By Price : " + layerType.toString() + "," + price);
    const layer: VectorFieldLayer | undefined = this.layers.get(layerType)

    if (layer != null) {
      //Logger.log("VectorFieldRow : Got Layer " + layerType.toString());
      return layer.getCellByPrice(price)
    }
    //    Logger.log("VectorFieldRow : Couldn't get Cell By Price : " + layerType.toString() + "," + price);
    return undefined
  }
  ...
  getCellByIndex(index: number): VectorFieldCell | undefined {
    //if (index % 10 === 0)
    // {
    //     Logger.log("VectorFieldRow : GetCellByIndex : Getting Cell For :" + index);
    // }

    //Logger.log(this);

    const adjustedIndex: number = index - this.presenter.numParticlesRemoved
    ...
    // Logger.log("VectorFieldRow : Can't get cell for :" + adjustedIndex);
    return undefined
  }

  getCellFromLayerByIndex(
    layertype: VectorFieldLayerType,
    index: number
  ): VectorFieldCell | undefined {
    //if (index % 10 === 0)
    ...
    const layer: VectorFieldLayer | undefined = this.getLayer(layertype)

    if (layer != null) {
      return layer.getCellByIndex(index)
    } else {
      Logger.log("VectorFieldRow : Can't get cell for :" + index)
      return undefined
    }
  }

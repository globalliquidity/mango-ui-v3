...
export class VectorFieldLayer {
  cellsByPrice: Map<currency, VectorFieldCell> = new Map<
    currency,
    VectorFieldCell
  >()
  cellsByIndex: Map<number, VectorFieldCell> = new Map<
    number,
    VectorFieldCell
  >()
  currentIndex: number
  ...
  getCellByIndex(index: number): VectorFieldCell | undefined {
    //Logger.log("VectorFieldLayer : Get Cell By Index :" + index);

    if (index >= this.startingIndex) {
      if (index <= this.endingIndex) {
        //Logger.log("VectorFieldLayer : Index is in range :" + index);
        return this.cellsByIndex.get(index)
      }
      //Logger.log("VectorFieldLayer : Can't get cell for :" + index);
    }
    //Logger.log("VectorFieldLayer : Can't get cell for :" + index);
    return undefined
  }
  ...
  getCellByPrice(price: currency): VectorFieldCell | undefined {
    ...
    for (const cell of Array.from(this.cellsByPrice.values())) {
      if (cell.price.value === price.value) return cell
    }
    //Logger.log("VectorFieldLayer :Couldn't Find Cell For price :" + price);
    //return null;
  }

  public addCell(price: currency, cell: VectorFieldCell) {
    if (this.cellsByPrice != null) this.cellsByPrice.set(price, cell)

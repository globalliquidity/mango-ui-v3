...
export class SolidParticleMaterial extends bjs.PBRMaterial {
  constructor(public name: string, public scene: Scene) {
    ...
    this.reflectionTexture = this.scene
      .hdrTexture as bjs.Nullable<bjs.BaseTexture>
    this.roughness = 0.2
    this.metallic = 0.2
    //this.environmentIntensity =2
    //this.subSurface.isTranslucencyEnabled = true;
    //this.clearCoat.isEnabled = true;
    //this.clearCoat.intensity = 0.5;
  }
  ...
  static setUVScale(
    mesh: bjs.Mesh | bjs.InstancedMesh,
    uScale: number,
    vScale: number
  ) {
    const UVs = mesh.getVerticesData(bjs.VertexBuffer.UVKind)
    ...
    if (UVs) {
      ...
      mesh.setVerticesData(bjs.VertexBuffer.UVKind, UVs)
    }
  }

  static setUVColorToMesh(
    mesh: bjs.Mesh | bjs.InstancedMesh,
    color: GLSGColor
  ) {
    if (mesh) {
      if (mesh.material) {
        ...
        textures.forEach((texture: bjs.BaseTexture) => {
          const tex: bjs.Texture = texture as bjs.Texture
          tex.uScale = UVColor.z - UVColor.x
          tex.vScale = UVColor.w - UVColor.y
          tex.uOffset = UVColor.x
          tex.vOffset = UVColor.y
        })
      }
    }
  }

...
export abstract class SolidParticleSystemElement extends SceneElement {
  ...
  constructor(
    ...
    Logger.log(
      ' SolidParticleSystemElement : Creating SPS with ' +
        this.particleCount +
        ' particles'
    )
  }
  ...
  public updateParticle = (particle: bjs.SolidParticle) => {
    //Logger.log('SolidParticleSystemElement : updateParticle()');
  ...
  protected initSPS() {
    Logger.log(
      'SolidParticleSysyem : initSPS() : ' + this.particleCount + ' particles.'
    )
    ...
    if (this.spsOptions !== undefined) {
      if (this.spsOptions.expandable) {
        this.sps = new bjs.SolidParticleSystem(
          this.name + '-sps',
          this.scene.bjsScene,
          { expandable: true }
        )
    ...
    this.meshBase.setEnabled(false)
    if (this.sps != null) {
      if (this.posOptions && this.posOptions.positionFunction) {
        this.sps.addShape(this.meshBase, this.particleCount, {
          positionFunction: this.posOptions.positionFunction,
        })
      } else {
        this.sps.addShape(this.meshBase, this.particleCount, {
          positionFunction: this.setInitialParticlePosition,
        })
      }
